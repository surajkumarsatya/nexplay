  ...
  return (
    ...
    >
      <CarouselContent className="">
        {data?.map((vid) => (
          <CarouselItem key={vid.id} className="w-full max-w-[900px] h-[500px]">
            <Link href={getWatchUrl(vid.id, vid.media_type, vid?.backdrop_path)}>
              <Image
      ...
      <div className="absolute !bottom-2 right-[12%] hidden md:flex" style={{bottom: "45px"}}>
        <CarouselPrevious className="w-[60px] h-[60px]" style={{marginLeft: "-30px"}}/>
        <CarouselNext className="w-[60px] h-[60px] !ml-3" />
      </div>
    </Carousel>
  )
}
